...
# Generate commands in the style of:
...
# python erc/exp/exp_uci.py --cfg_file=cfg_uci --cfg_dir=erc/config/exp_cp --exp_suffix=_TEST --no-get_pred --save_file --batch_ts=1 --tracker_window 0 0 0 --stop_counter 5 25 0 --device=cpu


# PARAM GRIDS
configs = [
    ('exp_ood', 'exp_ood', 'cfg_step'), 
    ('exp_ood', 'exp_ood', 'cfg_noshift'), 
    ('exp_ood', 'exp_ood', 'cfg_direct'),
    ('exp_cp', 'exp_cp', 'cfg_natural'),
    ('exp_uci', 'exp_cp', 'cfg_uci')
]
tracker_window = {
    "exp_ood": [0, 200, 50, 10],
    "exp_cp": [0, 365, 50, 10],
...
def main():
    ...
    with open(file, "w") as f:
        for (exp, cfg_dir, cfg) in configs:
            for bts in batch_ts:
                for tw in tracker_window[exp]:
                    s = f"python erc/exp/{exp}.py --cfg_file={cfg} --cfg_dir=erc/config/{cfg_dir} --exp_suffix={suffix} --no-get_pred --save_file --batch_ts={bts} --tracker_window 0 {tw} {tw} --stop_counter {sc[0]} {sc[1]} {sc[2]} --device={device}"
                    f.write(s + "\n")
    print("Done!")
